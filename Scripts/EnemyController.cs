...
public class EnemyController : MonoBehaviour
{
    ...
    public float level = 1; //Nivel 1 es normal, 2 es dificil y 0 niños 

    public bool acabarJuego;
        ...
        foreach (Transform nave_enemiga in Enemy)
        {
            if (!noDisparar)
            {
                ...
                if (contadorDisparo > cantidadDisparos)
                {
                    ...
                    contadorDisparo = 0;
                    FindObjectOfType<AudioManager>().Play("DisparoEnemigos");
                }
            }
